...
const defaultEmojiId = 'evergreen_tree';
...
class CSDialogTimer {
  ...
  async createShowElement () {
    ...
    const TEMPLATE = await this.templateWithEmojiSVG();
  ...
  findEmojiSVG (byId) {
    return this._icons.findEmojiSVG(byId || defaultEmojiId);
  }

  async templateWithEmojiSVG () {
    const { emojiId } = await this._storage.get();
    return this._template.replace(/<x-svg>.*<\/x-svg>/, this.findEmojiSVG(emojiId));
  }
